...
| | `codelldb`, `js-debug-adapter`, `debugpy` | Installed through Mason (`ensure_installed`). Rust debugs on `codelldb` through rustaceanvim, Python on `debugpy` through nvim-dap-python, JavaScript/TypeScript on `js-debug-adapter`. |
...
- **Indentation** — a `.clang-format` found upward from the file sets the width (`IndentWidth`, `UseTab`, `TabWidth`, or the `BasedOnStyle` default), and a project `.editorconfig` takes precedence over it. With neither, the buffer decides: if more of its first 2000 lines start with a tab than with two or more spaces, it gets 8-column tabs (`tabstop = shiftwidth = softtabstop = 8`, `noexpandtab`); otherwise 4 spaces (`shiftwidth = softtabstop = 4`, `expandtab`) with `tabstop = 8`, so a tab character is 8 columns wide either way. That fallback is what covers kernel trees older than v4.17, which ship no `.clang-format` (and no `.editorconfig` before v6.8). Only buffer options are set; the file's existing whitespace is never rewritten. This does not depend on clangd, so it holds where `lsp_filter` has turned the server off. `list = true` and `vim.b.autoformat = false` (no clang-format-on-save) apply in every case.
...
**Which languages are wired up, and by whom.** C, C++ and assembly go through GDB, and that half lives in `lua/plugins/dap.lua` and `lua/dbg/`. Everything else is wired the way its own ecosystem wires it, in `lua/plugins/dap_languages.lua`, and none of it goes through `lua/dbg/`:
...
| Python | [`nvim-dap-python`](https://github.com/mfussenegger/nvim-dap-python) on `debugpy` | Its own configurations (`file`, `file:args`, `attach`, `file:doctest`), venv detection, and debugging the test under the cursor |
...
| Elixir | [ElixirLS](https://github.com/elixir-lsp/elixir-ls)'s debug adapter, as a `mix_task` | `mix test` and `mix run` under the debugger, with the project's modules interpreted |

Adapters are resolved from Mason's package directory rather than `PATH`, because Mason only extends `PATH` once it has loaded. A language with no adapter installed is not special-cased at all: `<leader>dc` there produces nvim-dap's own *No configuration found for `<filetype>`* and nothing else happens. `.vscode/launch.json` is read on demand for every language, comments included. Julia is not offered: the debugger shipped with `julia-lsp` expects three named pipes and a VS Code handshake, which is not the standard DAP launch model.

Rust is the one case where the language's tooling reaches past the debugger: rustaceanvim starts and configures `rust-analyzer` itself, so `nvim-lspconfig`'s `rust_analyzer` is disabled to keep a second client off the same buffer. The LSP server is still the same `rust-analyzer` binary Mason installs.

**The panels follow the session's context, decided from the adapter, not hardcoded per language.** `lua/dbg/context.lua` classifies every session as `linux_kernel`, `native` (GDB on a userspace program) or `managed` (everything else), from the adapter type or an explicit `dbg_profile` on the configuration; the default is `managed`, so an adapter this config has never heard of gets the stock experience rather than the GDB one.

A GDB session gets the full panel set below and the winbar that names it — *Registers(G)*, *Memory(M)*, *Mappings(V)*, *Control flow(F)*, *Target(I)*, *gdb console(R)*. A managed session gets `nvim-dap-view` exactly as it ships: its own sections, its own labels (*Scopes*, *Watches*, *Exceptions*, *Breakpoints*, *Threads*, *Sessions*, *REPL*, *Console*) and its own keymap hints, with nothing renamed after GDB. None of the GDB machinery attaches to it — not the stop hooks, not the layout takeover, not the source-jump routing, not the window-resize handlers — and panels an earlier GDB session or `:DbgLayout` left on screen are dropped when it starts. Asking a managed session for a GDB-only view or command (Registers, Memory, Mappings, disassembly, the control-flow graph, `:DbgState`, `:DbgLayout`, `:DbgSafeMem`, `:DbgInline`, `<leader>dF`) is refused with a note naming what it is for. Within a GDB session the low-level panels still degrade rather than break — `:DbgState` reports which of them the target actually supports and why.
...
**The entry address comes from the run, not from the tree's default.** A tree states one boot mode and one entry address, but the same tree can be started in another — `--boot direct` on a tree whose default is `uefi`, say — and then that address is one the CPU never executes. The launcher records the mode and, for a firmware chain, the address the bootloader lands the kernel at, per gdb port; the adapter reads that and hands gdbtools the combination this port is actually running. A firmware boot also gets a hardware breakpoint, because the bootloader copies the image to that address *after* reset and a software breakpoint planted there while the guest is frozen is overwritten before the CPU arrives. A u-boot chain additionally gets the bootloader's own ELF loaded alongside `vmlinux`, so the pre-kernel stages are named rather than bare addresses; a UEFI chain gets no such symbols, because OVMF ships none to load. A direct boot has no recorded address at all: the entry is left unstated so gdbtools recovers or scans for it, which on x86 means running the decompressor and reading the base it chose. With nothing recorded for the port — a guest something else started — the tree's stated address is the only evidence there is, and it is used, on the assumption that the tree is running in the mode it says it defaults to.

One thing then takes that address away again, on x86_64 only: a guest whose KASLR state is not positively *off* leaves the entry unstated after all, because x86 randomizes the physical base and a stated address both misses and suppresses the recovery that would find the real one; an address exported by hand is left alone, since that is the documented way to override the recovery. arm64 and riscv64 keep their stated address, randomizing only the virtual base. KASLR itself is read from the recorded run first — a firmware chain passes the kernel command line itself, so there is no `-append` to read it from — then from the guest's command line, then from the build's `CONFIG_RANDOMIZE_BASE`, and a guest that answers none of those counts as randomized: running a recovery that was not needed costs a few seconds, while skipping one that was needed breaks at an address the kernel never uses. The shell launcher scores it in the same order, so a terminal session and an editor session cannot disagree about one guest.
...
**A gdbstub that dies says so instead of hanging.** Requests to a dead stub are simply never answered, so the QEMU process the target was discovered from is watched while the session runs. When it disappears the session is disconnected with the reason, and the panels stay up saying that what they show is the last state the target reported, not the current one — the same rule as every other way a session ends.

**Finding a target.** `<leader>dq` lists the QEMU instances currently exposing a gdbstub. Nothing is hardcoded: every candidate is built by reading `/proc/<pid>/cmdline` of each `qemu-system-*` process and parsing its actual `-gdb`, `-S`, `-kernel`, `-append`, `-M` and `-cpu` flags; the symbol file is derived from the `-kernel` path (stripping `arch/*/boot/*`, or using the image itself when it is an ELF, as for bare-metal targets); a guest booted through firmware carries no `-kernel` naming the Linux image — UEFI loads it from the ESP, and a riscv64 u-boot chain spends `-kernel` on u-boot itself — so every other filesystem path on the command line, the pflash and the disks included, is walked up from until a kernel build turns up beside one, and the candidate says which path it was found next to; the architecture comes from the symbol file's ELF `e_machine`; the port is confirmed against `/proc/net/tcp`, never by dialling it, because a QEMU gdbstub accepts a single client and an exploratory connection disturbs it. KASLR is reported from the run the launcher recorded for this port, falling back to the guest's own `-append` line and then to `CONFIG_RANDOMIZE_BASE` in the build's `.config`. `<leader>dQ` shows the same data as a table with the evidence for every field, and anything without evidence is reported as unknown rather than guessed.
...
**One core's page tables are not the guest's, and one page is not the read.** Where the core the session is stopped on cannot translate the address, the others are asked before the answer is "unmapped" — an SMP guest in early boot has cores in different regimes at once, and a secondary parked in `head.S` answers a flat `Unmapped` for every kernel address while the primary maps them all (measured on a v4.6 arm64 guest stopped at the second `__enable_mmu`: `cpu 1 Unmapped`, `cpu 0 gpa: 0x40082c00`, same address, same stop). The first page still decides how the window is read; the pages after it can only veto, since the hex view reads up to 8 KiB and a device region anywhere under that window is the hazard. The monitor is put back on the session's own core afterwards either way.
...
**The debug windows open with the session and are closed by hand.** Starting a session records the window layout first, then opens the panels. Ending one closes nothing: the last stop, the console and whatever the program printed are what you read afterwards, so the panels stay. `:DbgClose` puts the recorded layout, the focused window and its buffer back — so a session that walked into `libc_start_call_main.h` does not leave you there — and drops the sources the debugger listed on its way through, that glibc header among them, from the buffer list, unless you edited them or still have them open. `<leader>du` closes the panel without restoring the layout. Each panel also lives in exactly one window at a time: ask for Registers somewhere new and the old window gives it up, the side panel moving on to the first panel nothing else is showing.
...
| `<leader>dR` / `<leader>dm` / `<leader>dD` | Registers / hex view / disassembly — GDB sessions; refused with a note elsewhere |
| `<leader>ds` | Target panel — GDB sessions; refused with a note elsewhere |
| `<leader>dq` | Pick a QEMU gdbstub and attach |
| `<leader>dQ` | Report every debug target found on this host, with the evidence |
| `<leader>dPt` / `<leader>dPc` | Debug the Python test method / test class under the cursor (python buffers) |
...
- **Ending a session leaves the windows open.** The last stop, the console and whatever the program printed are what you read after it finishes, so nothing closes on its own — not when the program exits, not when you detach, not when a QEMU target dies under you. `<leader>du` closes the panel and `:DbgClose` puts the pre-session layout back; both are asked for.
- **Early-boot Linux Kernel tooling is not armed by default.** `:DbgKernel` leaves gdbtools' `kearly` commands registered but inert; `:DbgKernelEarly` exports `GDBTOOLS_AUTO=1` and runs `kearly on`, `kearly bootbreak`, `kearly status`. The armed mode installs stop hooks that resume the target and rewrite the symbol table on their own, which a DAP client does not model: `bootbreak` resumes the guest from inside the request that ran it, and the session catches up when it lands.
- **An x86 UEFI guest with KASLR on measures its entry, so it needs a QEMU monitor and a spare debug register per copy of the image.** The EFI stub picks the kernel's physical base itself, so neither the address the launcher recorded (measured without KASLR) nor the decompressor recovery (which looks for a bzImage loaded the way `-kernel` loads one) applies. gdbtools takes the stub's own hand-off instruction instead, where the kernel's absolute entry is already in a register, and to reach it has to find the loaded image first: the guest is advanced through QEMU's monitor in short steps and physical RAM is dumped and searched between them, with the guest frozen. Two things follow. The monitor is required, so this works on QEMU and not on a target reached some other way. And the firmware keeps two identical copies of the image, the loader's buffer and the one it executes, so the hand-off is armed in both — a hardware breakpoint each, out of the four x86 has, and more copies than that would be reported rather than silently dropped.
- **Arming takes seconds, and sometimes tens of them.** `kearly bootbreak` walks from the reset vector to the kernel entry, and the guest runs the whole way there under TCG — KVM is off on x86 because an early-boot hardware breakpoint is only deterministic without it. What it has to run through decides the cost. Measured on this machine, from attach to the entry:

  | Target | Arming | What the time is |
  |---|---|---|
  | x86_64, firmware boot | 3 s | firmware hands over at a known address |
  | arm64, u-boot | 3 s | same |
  | x86_64, direct boot | 15 s | the entry is not knowable until the decompressor has picked it, so the guest decompresses the whole kernel first |
  | arm64, direct boot | 32 s | no recorded load address, so the Image magic is scanned for over physical RAM |
  | riscv64, direct boot | 45 s | OpenSBI runs to the hand-off before the entry breakpoint can be reached |
  | x86_64, UEFI + KASLR | 34 s | the image has to be found in RAM first, so the guest is advanced in short steps and searched between them, and then it decompresses the whole kernel |

  The session looks stopped at the reset vector for that whole time. That is the arming, not a hang.
- **`<leader>dP` waits a moment in Python and Rust buffers.** It is Pause, and it is a prefix of `<leader>dPt` / `<leader>dPc` / `<leader>dPr`, so Neovim holds it for `timeoutlen` (300 ms here) in those buffers in case a third key follows. Nothing is mis-recognised. `<leader>dPt` and `<leader>dPc` are LazyVim's own keys for the Python functions; `<leader>dPr` is ours, put under the same prefix for symmetry (LazyVim itself binds Rust debuggables to `<leader>dr`).
