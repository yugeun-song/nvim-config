...
**One core's page tables are not the guest's.** Where the core the session is stopped on cannot translate the address, the others are asked before the answer is "unmapped" — an SMP guest in early boot has cores in different regimes at once, and a secondary parked in `head.S` answers a flat `Unmapped` for every kernel address while the primary maps them all (measured on a v4.6 arm64 guest stopped at the second `__enable_mmu`: `cpu 1 Unmapped`, `cpu 0 gpa: 0x40082c00`, same address, same stop). The monitor is put back on the session's own core afterwards either way.
